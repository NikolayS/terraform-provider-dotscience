...
install-release:
	...
	GO111MODULE=on go install -ldflags="$(LDFLAGS)" github.com/dotmesh-io/terraform-provider-dotscience/cmd/terraform-provider-dotscience

install:
	cd cmd/terraform-provider-dotscience && go install

plugin-install: install
	cp $(GOPATH)/bin/terraform-provider-dotscience $(HOME)/.terraform.d/plugins

test-install: install
	cp $(GOPATH)/bin/terraform-provider-dotscience example

test: test-install
	GO111MODULE=on go run cmd/test/main.go
